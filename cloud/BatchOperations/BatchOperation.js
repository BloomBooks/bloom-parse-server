...
const getCredentials = function (username, password) {
    return new Promise(function (resolve, reject) {
        if (username) {
            resolve(username);
        } else {
            console.log(
                "To update object(s) requires a logged in user that has permissions to modify the object. If in doubt, check the ACL and CLP."
            );
...
const batchUpdate = function (objects, tag) {
            ...
            new Promise(function (resolve, reject) {
                ...
                for (var j = i; j < i + 50 && j < objects.length; j++) {
                    requests.push({
                        ...
                        path:
                            "/parse/classes/" +
                            module.exports.classBeingBatchUpdated +
                            "/" +
                            objects[j].objectId,
                        body: module.exports.updateBodyForObject(
                            objects[j],
                            remainingArgs
                        ),
                    });
...
const findObjects = function (attr, regex) {
    return new Promise(function (resolve, reject) {
        var params = {};
...
const previewObjects = function (objects) {
    //If not forcing the update, display the targeted section of data
    if (mode == "-l" || mode == "-i") {
        if (mode == "-i") {
            console.log();
            console.log(
                "Will batch update the following " +
                    module.exports.classBeingBatchUpdated +
                    ":"
            );
        }
...
const batchOperation = function (args) {
    ...
    const exitWithUsage = function () {
        console.log(
            "usage: node " +
                programName +
                " [-u <username>] [-p <passwd>] [-e env] (-i | -l | -f) <attr> <regex> " +
                module.exports.usageAddendum
        );
        process.exit(1);
    };

    if (process.argv.length < 6) exitWithUsage();
    ...
    if (
        ...
        !attribute ||
        !regex ||
        !module.exports.verifyRemainingArguments(remainingArgs)
    ) {
        exitWithUsage();
    }
        ...
        .then(function (objects) {
            if (mode == "-l") {
                ...
                return module.exports
                    .prepareForOperation(remainingArgs)
                    .then(function () {
